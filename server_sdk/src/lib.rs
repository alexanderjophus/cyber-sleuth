...
fn build_router() -> pavex_matchit::Router<u32> {
    ...
    router.insert("/api/digimon_search", 2u32).unwrap();
    router
}
...
pub mod route_1 {
    ...
    pub async fn handler(v0: &pavex_tracing::RootSpan) -> pavex::response::Response {
        let mut v1 = app::routes::Digimon::new();
        let v2 = app::routes::Digimon::list(&mut v1);
        let v3 = match v2 {
            Ok(ok) => ok,
            Err(v3) => {
                return {
                    let v4 = app::routes::ListError::into_response(&v3);
